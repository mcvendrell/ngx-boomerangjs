...
// These are the original boomerang scripts plus History & SPA for modern apps support, but without integrity or crossOrigin attributes since those can
// vary based on hosting and build setup. Users can customize the script list by providing their own in the config if needed.
export function createDefaultBoomerangScripts(baseUrl = DEFAULT_BASE): ScriptDescriptor[] {
  return [
    ...
    { src: `${baseUrl}/plugins/history.js` },
    { src: `${baseUrl}/plugins/spa.js` },
